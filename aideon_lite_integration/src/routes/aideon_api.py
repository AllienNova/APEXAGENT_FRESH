...
# ============================================================================
# OPEN SOURCE AI PROVIDERS API
# ============================================================================

@aideon_bp.route('/ai/open-source/status', methods=['GET'])
def get_open_source_providers_status():
    """Get status of open-source AI providers"""
    try:
        from src.services.open_source_providers import get_open_source_providers_status
        status = get_open_source_providers_status()
        return jsonify({
            'success': True,
            'providers': status,
...
@aideon_bp.route('/ai/open-source/models', methods=['GET'])
def get_open_source_models():
    """Get all available open-source models"""
    try:
        from src.services.open_source_providers import get_open_source_models
...
@aideon_bp.route('/ai/open-source/metrics', methods=['GET'])
def get_open_source_metrics():
    """Get open-source AI performance metrics"""
    try:
        from src.services.open_source_providers import get_open_source_metrics
        metrics = get_open_source_metrics()
        return jsonify({
            'success': True,
            'metrics': metrics,
            'timestamp': datetime.now().isoformat()
...
@aideon_bp.route('/ai/open-source/generate', methods=['POST'])
def generate_open_source_response():
    """Generate response using open-source AI providers"""
    try:
        data = request.json
        prompt = data.get('prompt')
        model = data.get('model')
        provider = data.get('provider')  # 'together' or 'huggingface'
        ...
        import asyncio
        from src.services.open_source_providers import generate_open_source_response
        ...
        try:
            response = loop.run_until_complete(generate_open_source_response(
                prompt=prompt,
                model=model,
                provider=provider,
        ...
        logger.error(f"Open-source AI generation error: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e),
            'content': ''
...
@aideon_bp.route('/ai/providers/status', methods=['GET'])
def get_ai_providers_status():
    """Get status of all AI providers"""
    try:
        from src.services.ai_providers import get_ai_providers_status
...
@aideon_bp.route('/ai/providers/metrics', methods=['GET'])
def get_ai_performance_metrics():
    """Get AI system performance metrics"""
    try:
        from src.services.ai_providers import get_ai_performance_metrics
...
@aideon_bp.route('/ai/generate', methods=['POST'])
def generate_ai_response():
    ...
    try:
        ...
        # Import here to avoid circular imports
        import asyncio
        from src.services.ai_providers import generate_ai_response
