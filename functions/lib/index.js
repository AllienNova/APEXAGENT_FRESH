    ...
    try {
        ...
        res.json({
            ...
            user: {
                ...
                displayName: (userData === null || userData === void 0 ? void 0 : userData.displayName) || 'Demo User',
                credits: (userData === null || userData === void 0 ? void 0 : userData.credits) || 2847,
                usageToday: (userData === null || userData === void 0 ? void 0 : userData.usageToday) || 0.42
            }
        });
    }
    catch (error) {
...
// Dashboard endpoints
app.get('/dashboard/metrics', authenticateUser, async (req, res) => {
    try {
        const metrics = {
            aiPerformance: {
                value: '98.7%',
...
// Security endpoints
app.get('/security/status', authenticateUser, async (req, res) => {
    try {
...
app.get('/security/logs', authenticateUser, async (req, res) => {
    try {
        const logs = [
            {
                id: 1,
...
app.get('/system/status', authenticateUser, async (req, res) => {
    try {
        const systemStatus = {
            status: 'optimal',
            hybridProcessing: {
...
app.post('/chat/send', authenticateUser, async (req, res) => {
    try {
...
function getModelProvider(model) {
    ...
    if (anthropicModels.some(m => model.includes(m)))
        return 'anthropic';
    if (googleModels.some(m => model.includes(m)))
        return 'google';
    if (togetherModels.some(m => model.includes(m)))
        return 'together';
    return 'simulation'; // fallback to simulation
}
// OpenAI API integration
async function callOpenAI(model, message, options) {
    var _a, _b;
    ...
    try {
        ...
        return {
            ...
            cost: calculateCost(model, ((_b = data.usage) === null || _b === void 0 ? void 0 : _b.total_tokens) || 0)
        };
    }
    catch (error) {
...
async function callAnthropic(model, message, options) {
    var _a, _b, _c, _d;
    ...
    try {
        ...
        return {
            ...
            tokens: (((_a = data.usage) === null || _a === void 0 ? void 0 : _a.input_tokens) || 0) + (((_b = data.usage) === null || _b === void 0 ? void 0 : _b.output_tokens) || 0),
            cost: calculateCost(model, (((_c = data.usage) === null || _c === void 0 ? void 0 : _c.input_tokens) || 0) + (((_d = data.usage) === null || _d === void 0 ? void 0 : _d.output_tokens) || 0))
        };
    }
    catch (error) {
...
async function callGoogle(model, message, options) {
    var _a, _b;
    ...
    try {
        ...
        return {
            ...
            cost: calculateCost(model, ((_b = data.usageMetadata) === null || _b === void 0 ? void 0 : _b.totalTokenCount) || 0)
        };
    }
    catch (error) {
...
async function callTogether(model, message, options) {
    var _a, _b;
...
// Simulation fallback
function simulateResponse(model, message) {
    const responses = [
...
app.get('/chat/history', authenticateUser, async (req, res) => {
    try {
        ...
        const snapshot = await messagesRef.orderBy('timestamp', 'desc').limit(parseInt(limit)).get();
        const messages = snapshot.docs.map(doc => (Object.assign(Object.assign({ id: doc.id }, doc.data()), { timestamp: doc.data().timestamp.toDate().toISOString() })));
...
app.get('/models/available', authenticateUser, async (req, res) => {
    try {
        const models = {
            openai: {
                flagship: [
        ...
        const totalModels = Object.values(models).reduce((acc, provider) => acc + Object.values(provider).reduce((providerAcc, category) => providerAcc + category.length, 0), 0);
        res.json({
            success: true,
            models,
            summary: {
...
app.post('/agents/:agentId/task', authenticateUser, async (req, res) => {
    try {
...
app.post('/files/upload', authenticateUser, async (req, res) => {
    try {
...
// Artifacts endpoints
app.get('/artifacts', authenticateUser, async (req, res) => {
    try {
        const artifacts = [
            {
                id: 'artifact-1',
...
// Analytics endpoints
app.get('/analytics/overview', authenticateUser, async (req, res) => {
    try {
        const analytics = {
            usage: {
                totalRequests: 15247,
...
app.post('/tardis/analyze', authenticateUser, async (req, res) => {
    try {
        ...
        const analysis = {
            ...
            metadata: {
                processingTime: 3.2,
                dataPoints: (data === null || data === void 0 ? void 0 : data.length) || 1000,
...
app.get('/tardis/capabilities', authenticateUser, async (req, res) => {
    try {
        const capabilities = {
            analysis: [
...
app.get('/ws/connect', authenticateUser, async (req, res) => {
    try {
